...
export default class InformationModule implements OnApplicationBootstrap {
    ...
    async updateOnCondition(condition) {
        ...
        let batch = [];
        let count = 0;
        animeList = animeList.filter(anime => anime.episodes.length !== anime.episodes.filter(episode => episode.sources.length === scrapers.filter(scraper => !scraper.infoOnly && scraper.enabled).length).length);
